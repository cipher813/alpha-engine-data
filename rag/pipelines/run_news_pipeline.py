...
def main() -> int:
    ...
    # throttle-degrades-this-adapter posture, config#2813).
    #
    ...
    #
    # config#2943: --budget-profile picks which derivation's cap/floor matches
    # the CALLER's own execution-timeout envelope — 'weekly' (15600s cap, the
    # Saturday top-up's 6h envelope) or 'daily' (2700s cap, the daily delta's
    # 1h envelope). Both take the SAME input (len(tickers), the resolved corpus
    # scope) and are lockstep-guarded by tests/test_fetch_budget.py against
    # their respective envelope constant.
