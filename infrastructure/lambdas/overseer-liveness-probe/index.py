...
def _alert(
    ...
    """Format + send a watch-plane alert via flow-doctor Telegram.

    Lists NEW problems individually (capped at ``_MAX_NEW_LISTED``), then
    summarizes continuing ones as a count. The flow-doctor dedup key covers
    the full alert signature so a run with zero new problems is fully
    suppressed before this function is called — but if it IS called, the key
    includes the new-count + continuing-count so a resolved-and-replaced set
    still pages."""
    _MAX_NEW_LISTED = 5

    parts = [f"{total_problems} wiring/liveness issue(s)"]
    if new_problems:
        parts.append(f"{len(new_problems)} new")
    if continuing_count:
    ...
    lines = [
        ...
        f"{' · '.join(parts)} across the fleet watch plane "
        "(the WATCHERS' own wiring, NOT a pipeline failure):",
    ]

    for p in new_problems[:_MAX_NEW_LISTED]:
        lines.append(f"🆕 {p}")
    if len(new_problems) > _MAX_NEW_LISTED:
        lines.append(
            f"  … and {len(new_problems) - _MAX_NEW_LISTED} more new issue(s) — "
            "see CloudWatch logs for the full list."
        )
    ...
    dedup_fingerprint = hashlib.sha256(
        "\n".join(sorted(_stable_problem_key(p) for p in new_problems)
...
def handler(event: dict, context) -> dict:  # noqa: ARG001 — Lambda contract
    ...
    if problems:
        new_problems, continuing_problems, resolved_keys = _alert_diff(problems, previous_state)
        new_count = len(new_problems)
        continuing_count = len(continuing_problems)
        resolved_count = len(resolved_keys)

        if new_problems:
            logger.warning(
                "overseer liveness: %d new, %d continuing, %d resolved: %s",
                new_count, continuing_count, resolved_count, new_problems,
            )
            alerted = _alert(
                new_problems, continuing_count, resolved_count,
                len(problems), kill_switches, checks_run, checks_failed,
            )
            if alerted:
                # Persist the FULL current set so the next run can diff against it.
                new_state = {_stable_problem_key(p): p for p in problems}
                _save_alerted_state(s3, new_state)
        else:
            logger.info(
