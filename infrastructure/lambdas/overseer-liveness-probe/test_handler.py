...
def test_registry_declares_productive_when_for_alert_drain():
    """The live registry must declare productive_when for alert-drain — the
    code path is inert without it, and a future schema change could silently
    lose the contract."""
    import yaml
    reg = yaml.safe_load(
        (Path(__file__).resolve().parents[2] / "overseer" / "playbooks.yaml").read_text()
